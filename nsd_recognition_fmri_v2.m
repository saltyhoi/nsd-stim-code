...
p.REPEATTYPE = design.REPEATTYPE(start_idx:end_idx);
...
%% =========================================================
Screen('TextFont', p.window, 'Courier New')
Screen('TextSize', p.window, 12)
DrawFormattedText(p.window,'Waiting for scanner trigger (5)\n(ESC to abort)','center','center',[255 255 255], [],[],[],3);
...
for trial = 1:p.ntrials
    ...
    isRepeat = (p.ISOLD(trial) == 1);
    if isRepeat
        if p.REPEATTYPE(trial) == 1
            repeatType = 'easy repeat (within run / recent)';
        elseif p.REPEATTYPE(trial) == 2
            repeatType = 'hard repeat (across runs / first-seen earlier)';
        else
            repeatType = 'repeat (type unspecified)';
        end
        fprintf('Trial %d/%d | Image ID %d | REPEAT IMAGE | Type: %s\n', ...
            trial, p.ntrials, p.KID73(trial), repeatType);
    else
        fprintf('Trial %d/%d | Image ID %d | NEW IMAGE\n', ...
            trial, p.ntrials, p.KID73(trial));
    end
    ...
    while GetSecs - stim_onset < p.stimDur
        ...
        if resp ~= 0 && responded == 0
            ...
            if ~isnan(button)
                if responded == 1
                    p.CHANGEMIND(trial) = 1;
                    fprintf('  Trial %d | Response updated (participant changed mind).\n', trial);
                end
                ...
                currentIsOld = (button == 2);
                isCorrectNow = (p.ISOLD(trial) == currentIsOld);
    ...
    if ~isnan(p.BUTTON(trial))
        ...
        if p.ISCORRECT(trial)
            fprintf('  Trial %d FINAL RESULT: CORRECT\n', trial);
        else
            fprintf('  Trial %d FINAL RESULT: INCORRECT\n', trial);
        end
    else
        p.MISSINGDATA(trial)  = 1;
        fprintf('  Trial %d FINAL RESULT: NO RESPONSE\n', trial);
    end
    % if ~isnan(p.BUTTON(trial))
    %     p.ISOLDCURRENT(trial) = (p.BUTTON(trial) == 2);
    %     p.ISCORRECT(trial)    = (p.ISOLD(trial) == p.ISOLDCURRENT(trial));
